...
public class BoardWriteController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// *** enctype="multipar/form-data" 인코딩 미지정 형식 요청
			...
			//	 --> 해당 객채에 여러 값이 담겨 있고 이를 순서대로 얻어오는 방법을 제공
			...
			while(files.hasMoreElements()) { // 다음 요소가 있으면 true
				...
				if(original != null) { // 업로드된 파일이 있을 경우
									   // 현재 file에서 얻어온 name속성을 이용해
									   // 원본 또는 변경을 얻어왔을 때 값이 null이 아닌 경우
			...
			if(mode.equals("update")) { // 수정
				...
				if(result > 0) { // 성공
					...
					// write?mode=update&type=1&cp=1&no=1561
