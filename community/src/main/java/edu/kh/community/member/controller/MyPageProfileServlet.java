	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 try {
			// 프로필 이미지 업로드 -> 변경 
				...
				// 파라미터가 얻어와지지 않는 이유 
				// 1. enctype ="multipar/form-data" -> 인코딩이 안되어 있어서 파라미터가 인지되지 않음
				...
				// servers -> 서버 설정 선택 -> Overview -> server options // server 더블 클릭 시 이동 
				// -> Serve Modules without Publishing 체크
				//	(업로드 되는 파일이 wepapp 폴더 내부에 저장될 수 있음)
				
				// 2-2. memberProfile 폴더까지의 절대 경로 얻어오기
				HttpSession session = req.getSession(); // sessiom 얻어오는 것은 지장 없음(사용 가능)
...
//				System.out.println(mpReq.getFilesystemName("profileImage"));
				...
				// 단, x버튼이 클릭된 상태면 null을 가지게 한다.
				 ...
				 // ** 프로필 이미지 삭제 ***
				 // 1) "delete" input type ='hidden' 태그의 값(파라미터)
				 // (주의) multipart/form-data 형식의 요청을 처리 중이기 때문에 
				 // req를 이용해서 파라미터를 얻어올 수 있다.
				 // -> multipartRequest 이용하면 가능 !
