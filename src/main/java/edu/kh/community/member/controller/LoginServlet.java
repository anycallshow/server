...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// POST 방식 요청 시 문자 인코딩 서버 기본값으로 지정
		// -> 한글 깨짐 -> 문자 인코딩 변경 필요
		...
		try {
			...
			if(loginMember != null) { // 성공
				 ...
				 * 
				 * - 특정 주소 요청 시 마다 
				 *  해당 주소와 연관된 쿠키파일을 브라우저가 알아서 읽어옴
				 ...
				 * 
				 * 3) 이후 특정 주소 요청 시 
				 *    쿠키파일을 브라우저가 찾아 자동으로 요청에 실어서 보냄
				 *  
				 * 4) 서버는 요청에 실려온 쿠키파일을 사용함 
				 * */
				...
				if(req.getParameter("saveId") != null) {
					// 쿠키 파일을 30일 동안 유지 
					c.setMaxAge(60 * 60 * 24 * 30); // 1초단위 
				}else { 
					// 쿠키파일을 0초 동안 유지 
					// -> 기존에 존재하던 쿠키파일에 유지시간을 0초로 덮어씌움 
					// 		= 삭제 하겠다는 소리 
					c.setMaxAge(0); // 1초단위 
				}
